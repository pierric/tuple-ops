...
module Data.Tuple.Ops.Cons (cons) where
...
class ConsableR va rb where
    consR :: (Generic vb, Rep vb ~ D1 (MetaOfD1 (Rep vb)) rb) => va -> rb x -> vb -> ConsR va rb vb x
...
instance ConsableR va (C1 mc (S1 ms (URec b))) where
    consR a _ b = M1 (M1 (K1 a) :*: M1 (K1 b))
...
instance (Linearize b0, Linearize b1, 
    ...
    consR a _ b = M1 $ normalize $ linearize $ (M1 (K1 a) :: S1 MetaS (Rec0 va) x) :*: unM1 (unM1 (from b))
...
cons a b = to $ M1 $ consR a (unM1 $ from b) b
